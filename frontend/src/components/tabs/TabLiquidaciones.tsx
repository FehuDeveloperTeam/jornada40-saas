...
export default function TabLiquidaciones({
  liquidaciones, showLiqForm, setShowLiqForm, isGeneratingLiq,
  ...
  calcularValorHorasExtras, generarLiquidacion, descargarLiquidacionPDF, selectedEmpleado,
}: Props) {
  if (!showLiqForm) {
    return (
          ...
          <div className="bg-white rounded-[1.5rem] shadow-sm border border-slate-200 overflow-hidden">
            <table className="w-full text-left border-collapse">
              ...
              {liquidaciones.map(liq => (
                <tbody key={liq.id}>
                  <tr onClick={() => setExpandedLiqId(expandedLiqId === liq.id ? null : liq.id!)} className="border-b border-slate-100 hover:bg-slate-50 transition-colors cursor-pointer group">
                    ...
                    <td className="p-4 flex items-center justify-end gap-4">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); descargarLiquidacionPDF(liq.id!, liq.mes, liq.anio); }} className="p-2 bg-slate-100 text-slate-600 hover:bg-slate-900 hover:text-white rounded-lg transition-colors shadow-sm" title="Descargar Liquidación Oficial">
  ...
  return (
    <form id="liqForm" onSubmit={generarLiquidacion} className="max-w-4xl mx-auto bg-white p-8 rounded-[1.5rem] shadow-sm border border-slate-200 space-y-8">
          ...
          <div className="space-y-3">
            {haberesImponiblesList.map((item, index) => (
              <div key={index} className="flex gap-4">
                <input type="text" placeholder="Glosa (Ej: Bono Producción)" value={item.glosa} onChange={(e) => { const newL = [...haberesImponiblesList]; newL[index] = {...newL[index], glosa: e.target.value}; setHaberesImponiblesList(newL); }} className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 font-medium outline-none" />
                <input type="number" placeholder="Valor ($)" value={item.valor || ''} onChange={(e) => { const newL = [...haberesImponiblesList]; newL[index] = {...newL[index], valor: Number(e.target.value)}; setHaberesImponiblesList(newL); }} className="w-40 px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 font-medium outline-none text-right" />
          ...
          <div className="space-y-3">
            {horasExtrasList.map((item, index) => (
              <div key={index} className="flex gap-4 items-center">
                <input type="text" placeholder="Glosa" value={item.glosa} onChange={(e) => { const newL = [...horasExtrasList]; newL[index] = {...newL[index], glosa: e.target.value}; setHorasExtrasList(newL); }} className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 font-medium outline-none" />
                <div className="w-24 relative">
                  <span className="absolute text-xs text-slate-400 top-[-16px] left-1">Hrs</span>
                  <input type="number" placeholder="0" value={item.horas || ''} onChange={(e) => { const val = Number(e.target.value); const newL = [...horasExtrasList]; newL[index] = {...newL[index], horas: val, valor: calcularValorHorasExtras(val, newL[index].recargo)}; setHorasExtrasList(newL); }} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 font-bold text-slate-900 outline-none text-center" />
                </div>
                <div className="w-24 relative">
                  <span className="absolute text-xs text-slate-400 top-[-16px] left-1">% Recargo</span>
                  <input type="number" value={item.recargo} onChange={(e) => { const val = Number(e.target.value); const newL = [...horasExtrasList]; newL[index] = {...newL[index], recargo: val, valor: calcularValorHorasExtras(newL[index].horas, val)}; setHorasExtrasList(newL); }} className="w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 font-bold text-slate-900 outline-none text-center" />
                </div>
        ...
        {haberesNoImponiblesList.length === 0 ? (
          ...
          <div className="space-y-3">
            {haberesNoImponiblesList.map((item, index) => (
              <div key={index} className="flex gap-4">
                <input type="text" placeholder="Glosa (Ej: Colación, Movilización, Viático)" value={item.glosa} onChange={(e) => { const newL = [...haberesNoImponiblesList]; newL[index] = {...newL[index], glosa: e.target.value}; setHaberesNoImponiblesList(newL); }} className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 font-medium outline-none" />
                <input type="number" placeholder="Valor ($)" value={item.valor || ''} onChange={(e) => { const newL = [...haberesNoImponiblesList]; newL[index] = {...newL[index], valor: Number(e.target.value)}; setHaberesNoImponiblesList(newL); }} className="w-40 px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 font-medium outline-none text-right" />
