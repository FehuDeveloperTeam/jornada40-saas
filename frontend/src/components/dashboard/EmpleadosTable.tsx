  ...
  return (
    <div className="rounded-3xl p-7 min-h-[500px] overflow-hidden glass-card">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 flex-wrap">
        <div>
          <h2 className="text-lg font-bold" style={{ color: 'var(--c-text-1)' }}>Directorio de Empleados</h2>
        ...
        <div className="relative w-full md:w-64">
          <input
            ...
            style={{ background: 'var(--c-bg-input)', border: '1px solid var(--c-border-input)', color: 'var(--c-text-1)' }}
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          <svg fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"
            className="w-4 h-4 absolute left-3 top-2.5" style={{ color: 'var(--c-text-3)' }}>
      ...
      {selectedEmpleadosIds.length > 0 && (
        <div className="rounded-2xl p-4 mb-6 shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4"
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium" style={{ color: 'var(--c-text-2)' }}>¿Qué deseas generar?</span>
          </div>
          <button
      ...
      {empleados?.length === 0 ? (
        <div className="text-center py-16 rounded-2xl" style={{ border: '1px dashed var(--c-border-2)' }}>
          <p className="font-medium mb-4" style={{ color: 'var(--c-text-3)' }}>Aún no tienes empleados en esta empresa.</p>
        </div>
      ) : (
          ...
          <table className="w-full min-w-[700px] text-left border-collapse">
              ...
              <tr style={{ borderBottom: '1px solid var(--c-border)' }}>
                <th className="px-3 py-4 text-left">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--c-text-3)' }}>Acciones</span>
                  </div>
                </th>
                {['RUT', 'Nombre Completo', 'Email'].map(h => (
                  <th key={h} className="p-4 text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--c-text-3)' }}>{h}</th>
                ))}
                ...
                {[
                  ...
                  <th key={key} className="p-4 text-xs font-bold uppercase tracking-wider relative" style={{ color: 'var(--c-text-3)' }}>
                    ...
                    {openFilterDropdown === key && (
                      <div onClick={(e) => e.stopPropagation()}
                        ...
                        style={{ background: 'var(--c-bg-modal)', border: '1px solid var(--c-border-2)', boxShadow: '0 12px 32px rgba(0,0,0,0.4)' }}>
                        <label className="flex items-center p-2 rounded cursor-pointer" style={{ borderBottom: '1px solid var(--c-border)' }}>
                          ...
                          <span className="ml-3 font-semibold text-sm" style={{ color: 'var(--c-text-1)' }}>Seleccionar todos</span>
                        </label>
                        {items.map(item => (
                          <label key={String(item)} className="flex items-center p-2 rounded cursor-pointer"
                            ...
                            <span className="ml-3 text-sm capitalize" style={{ color: 'var(--c-text-2)' }}>{String(item).toLowerCase()}</span>
                          </label>
                        ))}
                      </div>
                    )}
                ...
                <th className="p-4 text-xs font-bold uppercase tracking-wider relative" style={{ color: 'var(--c-text-3)' }}>
            ...
            <tbody>
              {filteredEmpleados?.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-12 font-medium" style={{ color: 'var(--c-text-3)' }}>
                ...
                filteredEmpleados.map((emp) => (
                  <tr key={emp.id} className="transition-colors"
                    ...
                    <td className="p-4 font-mono text-sm" style={{ color: 'var(--c-text-2)', whiteSpace: 'nowrap' }}>{formatRut(emp.rut)}</td>
                    <td className="p-4 font-medium" style={{ color: 'var(--c-text-1)' }}>
                    ...
                    <td className="p-4 capitalize text-sm" style={{ color: 'var(--c-text-2)' }}>{emp.departamento?.toLowerCase() || 'No especificado'}</td>
                    <td className="p-4 capitalize text-sm" style={{ color: 'var(--c-text-2)' }}>{emp.cargo.toLowerCase()}</td>
                    <td className="p-4">
