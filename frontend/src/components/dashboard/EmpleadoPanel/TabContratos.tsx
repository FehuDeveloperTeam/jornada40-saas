    ...
    <form id="contratoForm" onSubmit={guardarContrato} onChange={() => setHayCambiosContrato(true)} className="max-w-4xl mx-auto space-y-8 pb-10">
      ...
      <div className="p-4 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center gap-4" style={{ background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.2)' }}>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold" style={{ color: 'var(--c-text-1)' }}>¿Tienes el contrato en papel?</p>
      ...
      <div className="p-6 rounded-2xl" style={{ background: 'var(--c-bg-card-2)', border: '1px solid var(--c-border)' }}>
        <h3 className="text-lg font-bold mb-4 pb-2" style={{ color: 'var(--c-text-1)', borderBottom: '1px solid var(--c-border)' }}>1. Condiciones Generales</h3>
        <div className="grid grid-cols-2 gap-6">
          <div>
            <label className="block text-xs font-semibold mb-1" style={{ color: 'var(--c-text-3)' }}>Tipo de Contrato</label>
            <select name="tipo_contrato" required value={contratoData.tipo_contrato || 'INDEFINIDO'} onChange={handleContratoChange} style={{ width:'100%', background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.625rem', padding:'0.625rem 0.75rem', color:'var(--c-text-1)', outline:'none', cursor:'pointer' }}>
              <option value="INDEFINIDO" style={{ background:'var(--c-bg-modal)' }}>Indefinido</option>
              <option value="PLAZO_FIJO" style={{ background:'var(--c-bg-modal)' }}>Plazo Fijo</option>
              <option value="OBRA_FAENA" style={{ background:'var(--c-bg-modal)' }}>Por Obra o Faena</option>
            </select>
          </div>
          <div>
          ...
          <div className="col-span-2">
            <label className="block text-xs font-semibold mb-2" style={{ color: 'var(--c-text-3)' }}>Funciones a Desempeñar (Opcional)</label>
            <p className="text-xs mb-2" style={{ color:'var(--c-text-3)' }}>Por defecto se incluirá un texto legal genérico. Si agregas ítems aquí, se listarán en el contrato.</p>
            {funciones.map((func, index) => (
              <div key={index} className="flex gap-2 mb-2">
                <input type="text" value={func} onChange={(e) => { const newF = [...funciones]; newF[index] = e.target.value; setFunciones(newF); setHayCambiosContrato(true); }} style={{ flex:1, background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.5rem', padding:'0.5rem 0.75rem', color:'var(--c-text-1)', outline:'none' }} placeholder="Ej: Atención a público y ventas..." />
      ...
      <div className="p-6 rounded-2xl" style={{ background: 'var(--c-bg-card-2)', border: '1px solid var(--c-border)' }}>
        <h3 className="text-lg font-bold mb-4 pb-2" style={{ color: 'var(--c-text-1)', borderBottom: '1px solid var(--c-border)' }}>2. Remuneraciones y Quincena</h3>
        <div className="grid grid-cols-2 gap-6">
          <div>
            <label className="block text-xs font-semibold mb-1" style={{ color:'var(--c-text-3)' }}>Día de Pago (Mensual)</label>
            <input type="number" min="1" max="31" name="dia_pago" required value={contratoData.dia_pago || 5} onChange={handleContratoChange} style={{ width:'100%', background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.625rem', padding:'0.625rem 0.75rem', color:'var(--c-text-1)', outline:'none' }} />
          </div>
          <div>
            <label className="block text-xs font-semibold mb-1" style={{ color:'var(--c-text-3)' }}>Gratificación Legal</label>
            <select name="gratificacion_legal" value={contratoData.gratificacion_legal || 'MENSUAL'} onChange={handleContratoChange} style={{ width:'100%', background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.625rem', padding:'0.625rem 0.75rem', color:'var(--c-text-1)', outline:'none', cursor:'pointer' }}>
              <option value="MENSUAL" style={{ background:'var(--c-bg-modal)' }}>Mensual (Art. 50 - 25% con tope)</option>
              <option value="ANUAL" style={{ background:'var(--c-bg-modal)' }}>Anual (Art. 47 - 30% utilidades)</option>
            </select>
          </div>
          ...
          <div className="col-span-2 flex items-center gap-3">
            ...
            <label className="font-semibold" style={{ color:'var(--c-text-2)' }}>El trabajador recibirá Anticipo Quincenal</label>
          </div>
          ...
          {contratoData.tiene_quincena && (
            <div className="col-span-2 grid grid-cols-2 gap-4 p-4 rounded-xl" style={{ background:'rgba(37,99,235,0.08)', border:'1px solid rgba(37,99,235,0.25)' }}>
              <div>
                ...
                <input type="number" min="1" max="31" name="dia_quincena" value={contratoData.dia_quincena || 15} onChange={handleContratoChange} style={{ width:'100%', background:'var(--c-bg-input)', border:'1px solid rgba(37,99,235,0.3)', borderRadius:'0.5rem', padding:'0.5rem 0.75rem', color:'#bfdbfe', outline:'none' }} />
              </div>
              <div>
      ...
      <div className="p-6 rounded-2xl" style={{ background: 'var(--c-bg-card-2)', border: '1px solid var(--c-border)' }}>
        ...
        <div className="mb-6">
          ...
          <select name="tipo_jornada" required value={contratoData.tipo_jornada || 'ORDINARIA'} onChange={handleContratoChange} style={{ width:'100%', background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.625rem', padding:'0.625rem 0.75rem', color:'var(--c-text-1)', outline:'none', cursor:'pointer' }}>
            <option value="ORDINARIA" style={{ background:'var(--c-bg-modal)' }}>Ordinaria (Asignar Horarios)</option>
            <option value="ART_22" style={{ background:'var(--c-bg-modal)' }}>Artículo 22 (Sin límite de horario)</option>
            <option value="OTRO" style={{ background:'var(--c-bg-modal)' }}>Otra (Redactar manualmente)</option>
          </select>
        </div>
        ...
        {contratoData.tipo_jornada === 'OTRO' && (
          <div className="p-4 rounded-xl mb-4" style={{ background:'rgba(37,99,235,0.08)', border:'1px solid rgba(37,99,235,0.25)' }}>
            ...
            <textarea name="jornada_personalizada" rows={3} value={contratoData.jornada_personalizada || ''} onChange={handleContratoChange} style={{ width:'100%', background:'var(--c-bg-input)', border:'1px solid rgba(37,99,235,0.3)', borderRadius:'0.5rem', padding:'0.5rem 0.75rem', color:'#bfdbfe', outline:'none', resize:'none' }} />
          </div>
        )}
        ...
        {contratoData.tipo_jornada === 'ORDINARIA' && (
          <div className="space-y-3 overflow-x-auto">
            ...
            <div className="grid grid-cols-12 gap-2 text-xs font-bold uppercase text-center px-2" style={{ color:'var(--c-text-3)' }}>
            ...
            {['lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado', 'domingo'].map((dia) => {
              ...
              return (
                <div key={dia} className="grid grid-cols-12 gap-2 items-center p-2 rounded-lg" style={{ background: datos.activo ? 'rgba(37,99,235,0.08)' : 'var(--c-bg-card-2)', border: datos.activo ? '1px solid rgba(37,99,235,0.2)' : '1px solid var(--c-border)', opacity: datos.activo ? 1 : 0.5 }}>
                  <div className="col-span-1 text-xs font-bold text-center" style={{ color:'var(--c-text-3)' }}>{dia.substring(0,2).toUpperCase()}</div>
                  <div className="col-span-3">
                    <label className="relative inline-flex items-center cursor-pointer">
                      ...
                      <div className="w-9 h-5 rounded-full peer peer-checked:after:translate-x-full after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-blue-600" style={{ background:'var(--c-border-2)' }}></div>
                    </label>
                  </div>
                  <div className="col-span-2"><input type="time" disabled={!datos.activo} value={datos.entrada} onChange={(e) => { setHorario({...horario, [dia]: {...datos, entrada: e.target.value}}); setHayCambiosContrato(true); }} style={{ width:'100%', fontSize:'0.75rem', padding:'0.25rem', background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.375rem', color:'var(--c-text-1)', outline:'none' }} /></div>
                  <div className="col-span-2"><input type="time" disabled={!datos.activo} value={datos.salida} onChange={(e) => { setHorario({...horario, [dia]: {...datos, salida: e.target.value}}); setHayCambiosContrato(true); }} style={{ width:'100%', fontSize:'0.75rem', padding:'0.25rem', background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.375rem', color:'var(--c-text-1)', outline:'none' }} /></div>
                  <div className="col-span-2"><input type="number" step="15" disabled={!datos.activo} value={datos.colacion} onChange={(e) => { setHorario({...horario, [dia]: {...datos, colacion: Number(e.target.value)}}); setHayCambiosContrato(true); }} style={{ width:'100%', fontSize:'0.75rem', padding:'0.25rem', background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.375rem', color:'var(--c-text-1)', outline:'none', textAlign:'center' }} /></div>
                  <div className="col-span-2 text-center font-mono font-bold" style={{ color: datos.activo ? '#93c5fd' : 'var(--c-text-3)' }}>{horasDia.toFixed(1)}h</div>
                </div>
              );
            })}
      ...
      <div className="p-6 rounded-2xl" style={{ background: 'var(--c-bg-card-2)', border: '1px solid var(--c-border)' }}>
        <h3 className="text-lg font-bold mb-4 pb-2" style={{ color: 'var(--c-text-1)', borderBottom:'1px solid var(--c-border)' }}>4. Cláusulas Adicionales</h3>
        {clausulas.map((clausula, index) => (
          <div key={index} className="flex gap-2 mb-3">
            <textarea rows={2} value={clausula} onChange={(e) => { const newC = [...clausulas]; newC[index] = e.target.value; setClausulas(newC); setHayCambiosContrato(true); }} style={{ flex:1, padding:'0.5rem 0.75rem', background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.5rem', color:'var(--c-text-1)', outline:'none', resize:'none' }} placeholder="Ej: Se acuerda un bono de productividad de..." />
      ...
      {contratoData.id && (
        <div className="rounded-2xl overflow-hidden" style={{ background:'var(--c-bg-card-2)', border:'1px solid var(--c-border)' }}>
          <div className="px-6 py-4" style={{ borderBottom:'1px solid var(--c-border)' }}>
            <h3 className="text-base font-bold" style={{ color: 'var(--c-text-1)' }}>5. Documentos del Contrato</h3>
            <p className="text-xs mt-0.5" style={{ color:'var(--c-text-3)' }}>Genera y almacena los PDFs oficiales. Puedes regenerarlos si actualizas los datos.</p>
          </div>
          ...
          <div className="p-6 space-y-4">
            <div className="p-4 rounded-xl" style={{ background:'var(--c-bg-card-2)', border:'1px solid var(--c-border)' }}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: 'var(--c-text-1)' }}>Contrato Laboral</p>
                  <p className="text-xs" style={{ color:'var(--c-text-3)' }}>{contratoData.tiene_contrato_pdf ? 'PDF generado ✓' : 'Sin PDF generado'}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
              ...
              <div className="mt-3 pt-3 flex items-center gap-2" style={{ borderTop: '1px solid var(--c-border)' }}>
                <FirmaBadge
                  tipo="CONTRATO"
            ...
            <div className="p-4 rounded-xl" style={{ background:'var(--c-bg-card-2)', border:'1px solid var(--c-border)' }}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: 'var(--c-text-1)' }}>Anexo Ley 40 Horas</p>
                  <p className="text-xs" style={{ color:'var(--c-text-3)' }}>{contratoData.tiene_anexo_40h_pdf ? 'PDF generado ✓' : 'Sin PDF generado'}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
      ...
      <div className="max-w-4xl mx-auto mt-8 pb-10">
        <div className="p-6 rounded-2xl" style={{ background:'var(--c-bg-card-2)', border:'1px solid var(--c-border)' }}>
          <div className="flex justify-between items-center pb-4 mb-4" style={{ borderBottom:'1px solid var(--c-border)' }}>
            <div>
              <h3 className="text-lg font-bold" style={{ color: 'var(--c-text-1)' }}>5. Anexos de Contrato</h3>
              <p className="text-sm mt-0.5" style={{ color:'var(--c-text-3)' }}>Modificaciones contractuales formalizadas por escrito.</p>
            </div>
          ...
          {showAnexoContratoForm ? (
            ...
            >
              <div className="grid grid-cols-2 gap-5">
                <div className="col-span-2">
                  <label className="block text-xs font-semibold mb-1" style={{ color:'var(--c-text-3)' }}>Título del Anexo</label>
                  <input type="text" required value={anexoContratoData.titulo || ''} onChange={e => setAnexoContratoData({ ...anexoContratoData, titulo: e.target.value })} placeholder="Ej: Modificación de cargo y remuneración" style={{ width:'100%', background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.625rem', padding:'0.625rem 0.75rem', color:'var(--c-text-1)', outline:'none' }} />
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1" style={{ color:'var(--c-text-3)' }}>Fecha de Emisión</label>
                  <input type="date" required value={anexoContratoData.fecha_emision || ''} onChange={e => setAnexoContratoData({ ...anexoContratoData, fecha_emision: e.target.value })} style={{ width:'100%', background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.625rem', padding:'0.625rem 0.75rem', color:'var(--c-text-1)', outline:'none' }} />
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1" style={{ color:'var(--c-text-3)' }}>Antecedentes (opcional)</label>
                  <textarea rows={2} value={anexoContratoData.descripcion || ''} onChange={e => setAnexoContratoData({ ...anexoContratoData, descripcion: e.target.value })} placeholder="Contexto o motivo del anexo..." style={{ width:'100%', background:'var(--c-bg-input)', border:'1px solid var(--c-border-input)', borderRadius:'0.5rem', padding:'0.5rem 0.75rem', color:'var(--c-text-1)', outline:'none', resize:'none' }} />
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => { setShowAnexoContratoForm(false); setClausulasAnexo([]); }} className="px-4 py-2 text-sm rounded-lg transition-colors" style={{ color:'var(--c-text-2)' }} onMouseEnter={e=>(e.currentTarget.style.background='var(--c-bg-input)')} onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
                  Cancelar
                </button>
            ...
            <p className="text-sm text-center py-8" style={{ color:'var(--c-text-3)' }}>No hay anexos de contrato registrados.</p>
          ) : (
            <div className="space-y-2">
              {anexosContrato.map(anexo => {
                ...
                return (
                  <div key={anexo.id} className="p-4 rounded-xl transition-colors" style={{ border:'1px solid var(--c-border)' }} onMouseEnter={e=>(e.currentTarget.style.background='var(--c-bg-hover)')} onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
