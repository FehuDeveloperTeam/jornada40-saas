...
- **Registro electrónico laboral (Ley 21.327)** — `core/registro_dt.py` (official tables and CSV) and `views/direccion_trabajo.py` (`/api/registro-dt/`), screen `/app/dt`. Mi DT has no API: Jornada40 computes what to register and by when, builds the bulk file, and records what the employer registered (`RegistroDT`, one row per `clave`: `CONTRATO:<id>`, `ANEXO:<id>`, `ANEXO40H:<solicitud>`, `TERMINO:<empleado>:<aaaammdd>`).
  ...
  - `GET /api/registro-dt/csv/` (Pyme+): ZIP with one CSV per cargo (the DT loads by cargo) named `rutempleador_aaaamm.csv`, columns and codes of the official "Instructivo de Registro Masivo de Contratos de Trabajo" v21.04.22 (`COLUMNAS`, Tabla 1 comunas, AFP/salud codes), `;`-separated, cp1252, CRLF, plus `LEEME.txt` with per-worker warnings (missing comuna code, unsplittable address, etc.). The official template is only downloadable inside Mi DT: the column order follows the guide and must be confirmed with the first real upload. Assumptions: Chilean RUT, no subcontracting/EST, workplace = company comuna, `OTROS_*` = 0.
