...
- **Registro electrónico laboral (Ley 21.327)** — `core/registro_dt.py` (official tables and CSV) and `views/direccion_trabajo.py` (`/api/registro-dt/`), screen `/app/dt`. Mi DT has no API: Jornada40 computes what to register and by when, builds the bulk file, and records what the employer registered (`RegistroDT`, one row per `clave`: `CONTRATO:<id>`, `ANEXO:<id>`, `ANEXO40H:<solicitud>`, `TERMINO:<empleado>:<aaaammdd>`).
  - Deadlines in **días hábiles** (Mon–Fri without holidays, the strictest reading): contract and anexo 15 from their celebration (electronic signature date; without it, the contract's start date; an anexo exists only once signed); termination 10 (Art. 159 N°1-3), 3 (159 N°4-5 and 160), 6 (159 N°6 and 163 bis), Art. 161 at the date of the notice letter. Termination data come from the finiquito, else the DESPIDO letter, else `fecha_desvinculacion` (no causal → 3 days).
  - **Ficha** (`GET /api/registro-dt/ficha/?empresa=&clave=`, "Ver ficha" on `/app/dt`): the data for Mi DT's individual form in the order of its 4 stages (Guía rápida DT 10.01.2024, DS N°14/2023), with copy buttons; the clauses the form asks to paste (remuneraciones, distribución de jornada, otras estipulaciones) are extracted from the contract Jornada40 renders (`_clausulas_del_contrato`). Anexo and termination fichas carry the usual data (their forms aren't documented field by field). This is the main path: a persona natural employer's Mi DT only offers the individual form (verified with a real account).
