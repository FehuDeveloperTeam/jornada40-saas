...
- **Reveniu API**: header `Reveniu-Secret-Key` = `REVENIU_API_KEY` or, by default, `REVENIU_WEBHOOK_SECRET` (same key). Base URL `REVENIU_API_URL` (default `https://api.reveniu.com`; sandbox `https://integration.reveniu.com`, account at `https://sandbox.reveniu.com`, test cards in docs). On a plan/cycle change the previous subscription gets `POST /api/v1/subscriptions/{id}/disablerenew/` (stays active until its next charge, no double charge); if the API fails, the cancel-by-hand email is sent.
